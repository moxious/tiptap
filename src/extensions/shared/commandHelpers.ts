 ...
 */
export function createToggleInlineNodeCommand(
  nodeName: string,
  defaultAttrs: Record<string, any> = {}
) {
  return () => ({ commands, state, chain }: any) => {
...
export function createSetInlineNodeCommand(
  ...
  return (attributes: Record<string, any> = {}) => ({ commands, state }: any) => {
